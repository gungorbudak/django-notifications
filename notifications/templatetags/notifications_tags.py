...
def live_notify_badge(context,badge_id='live_notify_badge',classes="",empty_zero=True):
    user = user_context(context)
    if not user:
        return ''
    unread = user.notifications.unread().count()
    if unread == 0 and empty_zero:
        html="<span id='{badge_id}' class='{classes}'></span>".format(badge_id=badge_id,classes=classes)
        return html
